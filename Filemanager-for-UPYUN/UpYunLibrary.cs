    ...
    public class UpYun
    {
        ...
        */
        ...
        private void upyunAuth(Hashtable headers, string method, string uri, HttpWebRequest request)
        {
            ...
            //request.Date = dt;
            headers.Add("Date", date);//modified
            string auth;
        ...
        private HttpWebResponse newWorker(string method, string Url, byte[] postData, Hashtable headers)
        {
            ...
            {
                string k = var.Key.ToString();
                string v = var.Value.ToString();
                if (WebHeaderCollection.IsRestricted(k))//added
                {
                    MethodInfo priMethod = request.Headers.GetType().GetMethod("AddWithoutValidate", BindingFlags.Instance | BindingFlags.NonPublic);
                    priMethod.Invoke(request.Headers, new[] { k, v });
                }
                else
                {
        ...
        */
        public List<FolderItem> readDir(string url)
        {
            ...
            StreamReader sr = new StreamReader(resp.GetResponseStream(), Encoding.UTF8);
            string strhtml = sr.ReadToEnd();
            if (String.IsNullOrEmpty(strhtml))//added
            {
                return null;
            }
            resp.Close();
